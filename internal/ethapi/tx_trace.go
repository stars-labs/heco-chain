...
func getStructLogForTransaction(
	...
	header *types.Header,
	block *types.Block,
	index uint64) (*TraceStructLogger, *types.Message, *core.ExecutionResult, error) {
	...
	result, err := core.ApplyMessage(evm, replayMsg, gp)
...
func traceTx(ctx context.Context, state *state.StateDB, header *types.Header, backend Backend, block *types.Block, tx *types.Transaction, index uint64) (*[]ActionTrace, error) {
...
func traceBlock(ctx context.Context, block *types.Block, backend Backend, txHash *common.Hash) (*[]ActionTrace, error) {
	var (
		mainErr       error
		blockNumber   int64
	...
	for _, tx := range block.Transactions() {
		if txHash == nil || *txHash == tx.Hash() {
			...
			tx, _, _, index, err := backend.GetTransaction(ctx, tx.Hash())
			if err != nil {
...
func (s *PublicTxTraceAPI) Transaction(ctx context.Context, hash common.Hash) (*[]ActionTrace, error) {
	...
	_, _, blockNumber, _, _ := s.b.GetTransaction(ctx, hash)
...
blocks:
	...
	// when timeout occurred or another error
